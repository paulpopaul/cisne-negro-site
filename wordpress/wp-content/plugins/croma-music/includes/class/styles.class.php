	...
	class Dynamic_Styles {
		...
		protected function importGoogleFonts() {
			...
			foreach($this->google_fonts as $font) {
				$currentFont = urlencode($font['font-family']);
				
				if( is_string($font['font-weight']) ){

					$fontWeight = explode(',', $font['font-weight']);
					$fontWeight = array_unique($fontWeight);
					$fontWeight = array_diff($fontWeight, array('Array'));
					$fontWeight = implode(",", $fontWeight);
					$currentFont .= ':' . $fontWeight;
				}

				echo "@import url(//fonts.googleapis.com/css?family=". $currentFont .");";
			}
				echo "\r\n";
		}
