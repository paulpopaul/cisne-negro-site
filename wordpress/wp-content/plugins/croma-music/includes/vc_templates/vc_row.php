...
if ( ! empty( $iron_croma_full_height ) ) {
	$iron_croma_css_classes[] = ' vc_row-o-full-height';
	if ( ! empty( $iron_croma_columns_placement ) ) {
		$iron_croma_flex_row = true;
		$iron_croma_css_classes[] = ' vc_row-o-columns-' . $iron_croma_columns_placement;
	}
}
...
if ( ! empty( $iron_croma_disable_element ) ) {
    if ( 'yes' === $iron_croma_disable_element ) {
        if ( vc_is_page_editable() ) {
            $iron_croma_css_classes[] = 'vc_hidden-lg vc_hidden-xs vc_hidden-sm vc_hidden-md';
        } else {
            return '';
        }
    }
...
if(!empty($iron_croma_iron_bg_video)) {
	...
	$iron_croma_override_bg_image = false;
	...
	if (!$detect->isMobile() && !$detect->isTablet()) {
		...
		$iron_croma_bg_video = '<div class="bg-video-wrap">';
			...
			if(!empty($iron_croma_iron_bg_video_webm)) {
				$iron_croma_bg_video .= '<source type="video/webm" src="'.$iron_croma_iron_bg_video_webm.'">';
			}
...
$iron_croma_custom = vc_settings()->get( 'row_css_class' );
if( empty($iron_croma_custom)){
	$iron_croma_custom = 'vc_row-fluid';
}

$iron_croma_css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG,  implode( ' ', array_filter( $iron_croma_css_classes )).' '.'vc_row wpb_row '. ( $this->settings('base')==='vc_row_inner' ? 'vc_inner ' : '' ) . $iron_croma_custom . $iron_croma_el_class . vc_shortcode_custom_css_class( $iron_croma_css, ' ' ), $this->settings['base'], $iron_croma_atts ).' '.$iron_croma_iron_row_type.$iron_croma_iron_parallax.$iron_croma_iron_bg_video;


if(!empty($iron_croma_iron_remove_padding_medium)) {

	$iron_croma_css_class .= ' tabletnopadding';
}

if(!empty($iron_croma_iron_remove_padding_small)) {

	$iron_croma_css_class .= ' mobilenopadding';
}

$iron_croma_output = '<div '.(!empty($iron_croma_iron_id) ? 'id="'.$iron_croma_iron_id.'"' : '').' class="' . $iron_croma_css_class . ' ' . $this->getCSSAnimation( $iron_croma_css_animation ) . '">';
    ...
    if( !empty($iron_croma_iron_overlay_color) || !empty($iron_croma_iron_overlay_pattern)){
        $iron_croma_output .= '<div class="background-overlay" style="'.$iron_croma_padding.' '.$iron_croma_iron_overlay_color.' '.$iron_croma_iron_overlay_pattern.';"></div>';
    }
